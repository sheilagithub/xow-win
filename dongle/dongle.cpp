...
{
    // The wire structs rely on GCC's (Linux) bit-field layout. MinGW's default
    // MS layout silently breaks them; the Makefile passes -mno-ms-bitfields.
    static_assert(sizeof(FrameControl) == 2, "FrameControl must be 2 bytes: build with -mno-ms-bitfields");
    static_assert(sizeof(WlanFrame) == 24, "WlanFrame must be 24 bytes: build with -mno-ms-bitfields");
    static_assert(sizeof(RxWi) == 32, "RxWi must be 32 bytes");
    static_assert(sizeof(RxInfoCommand) == 4, "RxInfoCommand must be 4 bytes");
...
void Dongle::handleWlanPacket(const Bytes &packet)
{
    ...
    Log::debug(
        "RX frame type %d subtype %d from %s to %s (wcid %d, %d bytes)%s",
        type,
        subtype,
        Log::formatBytes(source).c_str(),
        Log::formatBytes(destination).c_str(),
        rxWi->wcid,
        static_cast<int>(packet.size()),
        destination != macAddress ? " [not for us]" : ""
    );
