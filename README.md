...
- **Trusted Root certificate.** `dist\winusb-driver\` is a WinUSB driver package
  generated by Zadig (libwdi) and signed with a self-signed certificate named
  "USB\VID_045E&PID_02E6 (libwdi autogenerated)". The installer adds that
  certificate to the machine's Trusted Root and Trusted Publisher stores so
  Windows accepts the package. libwdi discards the private key after signing,
  but you are trusting a certificate you did not generate. If you prefer not
  to, skip the installer's driver step and bind the adapter with `zadig.exe`
  yourself, which generates a fresh certificate on your own machine.
- **Elevated background task.** The login task runs xow-win with administrator
  rights so it can cycle the adapter's USB port and re-apply the WinUSB
  binding. Everything it does is logged to `xow-win.log`.
- **Unsigned binary.** `xow-win.exe` is not code-signed. Windows Smart App
  Control blocks it entirely; SmartScreen may warn on first run. Build it from
  this source if you would rather not run a binary from someone else.
- **Anti-cheat.** Games see a ViGEmBus virtual Xbox 360 controller. This is the
  same driver DS4Windows and similar tools use and is broadly tolerated, but no
  guarantee is made for any particular anti-cheat system.
